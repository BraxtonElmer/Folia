"""
Seed the Supabase database with realistic canteen data.
...
async def seed():
    print("🌱 Starting ZeroWaste database seed...")
    ...
    # 3. Insert waste logs in batches
    print(f"\n[3] Inserting {len(data['waste_logs'])} waste log entries...")
    ...
    # Stats
    total_prepared = sum(l["prepared_qty"] for l in logs_to_insert)
    total_wasted = sum(l["leftover_qty"] for l in logs_to_insert)
    
    print(f"\n{'='*50}")
    print(f"🎉 Seed complete!")
    print(f"   📊 {inserted} waste log entries")
    print(f"   🍽️  {total_prepared:,} total preparations")
    print(f"   🗑️  {total_wasted:,} total wasted ({total_wasted/max(total_prepared,1)*100:.1f}%)")
    print(f"   💰 Est. cost wasted: ₹{total_wasted * 35:,.0f}")
    print(f"{'='*50}")
